...
public class JPAConfig {
    ...
    static {
        ...
        try (InputStream input =
                JPAConfig.class.getClassLoader().getResourceAsStream("database.properties")) {
            if (input != null) {
                dbProperties.load(input);
                System.out.println("✅ Loaded database.properties successfully!");
            } else {
                System.err.println(
                        "⚠️ database.properties not found! Using default values from persistence.xml");
            }
        } catch (IOException e) {
    ...
    /**
     * Chuẩn hóa auth mode Dự án chỉ hỗ trợ 2 chế độ: - "windows": Windows Authentication - "sql":
    ...
    private static Map<String, String> buildPersistenceProperties() {
        ...
        props.put("hibernate.show_sql", getProperty("hibernate.show_sql", "true"));
        props.put("hibernate.format_sql", getProperty("hibernate.format_sql", "true"));
